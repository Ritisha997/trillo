export const customAminoTypes = {
	"/comdex.vault.v1beta1.MsgCreateRequest": {
		...
		toAmino: ({ from, appId, extendedPairVaultId, amountIn, amountOut }) => {
			return {
				from,
				app_id: String(appId),
	...
	"/comdex.vault.v1beta1.MsgDepositRequest": {
		...
		toAmino: ({ from, appId, extendedPairVaultId, userVaultId, amount }) => {
			return {
				from,
				app_id: String(appId),
				extended_pair_vault_id: String(extendedPairVaultId),
	...
	"/comdex.vault.v1beta1.MsgCloseRequest": {
		...
		toAmino: ({ from, appId, extendedPairVaultId, userVaultId }) => {
			return {
				from,
				app_id: String(appId),
				extended_pair_vault_id: String(extendedPairVaultId),
		...
		fromAmino: ({ from, app_id, extended_pair_vault_id, user_vault_id }) => {
			return {
				from,
				appId: Number(app_id),
				extendedPairVaultId: Number(extended_pair_vault_id),
	...
	"/comdex.locker.v1beta1.MsgCreateLockerRequest": {
		...
		toAmino: ({ depositor, amount, assetId, appId }) => {
			return {
				depositor,
				amount: amount,
		...
		fromAmino: ({ depositor, amount, asset_id, app_id }) => {
			return {
				depositor,
				amount: amount,
	...
	"/comdex.locker.v1beta1.MsgAddWhiteListedAssetRequest": {
		...
		toAmino: ({ from, assetId, appMappingId }) => {
			return {
				from,
				asset_id: String(assetId),
		...
		fromAmino: ({ from, asset_id, app_mapping_id }) => {
			return {
				from,
				assetId: Number(asset_id),
	...
	"/comdex.locker.v1beta1.MsgDepositAssetRequest": {
		...
		toAmino: ({ depositor, lockerId, amount, assetId, appId }) => {
			return {
				depositor,
		...
		fromAmino: ({ depositor, amount, locker_id, asset_id, app_id }) => {
			return {
				depositor,
	...
	"/comdex.auction.v1beta1.MsgPlaceSurplusBidRequest": {
		...
		toAmino: ({ bidder, amount, auctionId, appId, auctionMappingId }) => {
			return {
				bidder,
				amount: amount,
		...
		fromAmino: ({ bidder, amount, auction_id, app_id, auction_mapping_id }) => {
			return {
				bidder,
				amount: amount,
	...
	"/comdex.locker.v1beta1.MsgCloseLockerRequest": {
		...
		toAmino: ({ depositor, appId, assetId, lockerId }) => {
			return {
				depositor,
				app_id: String(appId),
				asset_id: String(assetId),
		...
		fromAmino: ({ depositor, app_id, asset_id, locker_id }) => {
			return {
				depositor,
				appId: Number(app_id),
				assetId: Number(asset_id),
	...
	"/comdex.locker.v1beta1.MsgLockerRewardCalcRequest": {
		...
		fromAmino: ({ from, app_id, locker_id }) => {
			return {
				from,
				appId: Number(app_id),
				lockerId: Number(locker_id),
	...
	"/comdex.vault.v1beta1.MsgVaultInterestCalcRequest": {
		...
		toAmino: ({ from, appId, userVaultId }) => {
			return {
				from,
				app_id: String(appId),
				user_vault_id: String(userVaultId),
		...
		fromAmino: ({ from, app_id, user_vault_id }) => {
			return {
				from,
				appId: Number(app_id),
				userVaultId: Number(user_vault_id),
	...
	"/comdex.vault.v1beta1.MsgDepositStableMintRequest": {
		...
		toAmino: ({ from, appId, extendedPairVaultId, amount, stableVaultId }) => {
			return {
				from,
				app_id: String(appId),
				extended_pair_vault_id: String(extendedPairVaultId),
